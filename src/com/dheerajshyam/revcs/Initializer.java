...
final public class Initializer {
	...
	private File initFolder, indexFile, mainFile, snapsFolder;
	...
	public Initializer() {
		...
		mainFile = new File(initFolder.getPath() + "/main");
		
		if(!mainFile.exists()) {
			try {
				boolean isMainFileCreated = mainFile.createNewFile();
				if(!isMainFileCreated)
					throw new Exception();
			} catch(Exception e) {
				System.err.println("internal error occurred, unable to initiate the capsule.");
				System.exit(-1);
