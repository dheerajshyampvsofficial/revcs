...
final class MainObject implements Serializable {
	
	private static final long serialVersionUID = 3514087643625887117L;
	
	public String last_snapshot, last_snapshot_hash;
	
	public static boolean serialize(MainObject mainObject,
		File outputFile, Key secureKey) {
		...
		try {
			...
			SealedObject sealedNode = new SealedObject(mainObject, cipher);
	...
	public static MainObject deserialize(String filePath) {
		...
		key = EncryptionBuilder.generateKeyFromPath("./.revcs/" + secureKeyName);
		
		MainObject node = null;
		...
		if(key != null) {
			try (ObjectInputStream ois = new ObjectInputStream(
				...
				SealedObject sealedNode = (SealedObject) ois.readObject();
	            node = (MainObject) sealedNode.getObject(key);
...
final public class Storing {
	...
	public void store(String snap_name, File snapFolder, Key secureKey) {
		
		MainObject mainObject = new MainObject();
		mainObject.last_snapshot = snap_name;
		...
		if(rootNode.children.isEmpty())
			...
			class ObjectsBuilder {
				...
				private FileObject build_file_object(File file) {
					...
					try {
						...
						e.printStackTrace();
						System.err.println("Object building issue.");
						System.exit(-1);
					}
			...
			new ObjectsBuilder().build_objects(rootNode);
			...
			for(FileObject fileObject : fileObjects) {
				...
				if(!fileObjectFile.exists()) {
					try {
						...
						mainObject.last_snapshot_hash += HashBuilder.getHashFromBytes(fileObject.data);
		...
		File mainFile = new File("./.revcs/main");
		if(mainFile == null || !mainFile.exists()) {
			DirectoryDeleter.delete(snapFolder);
			System.out.println("Main file does not exists.");
			System.exit(-1);
		}
		
		boolean isMainObjectSerialized = MainObject.serialize(mainObject, mainFile, secureKey);
		if(!isMainObjectSerialized) {
			DirectoryDeleter.delete(snapFolder);
			System.out.println("Unable to serialize main object.");
			System.exit(-1);
		}
		...
		try {
			...
			if(indexFile.length() == 0) {
				...
				if(this.rootNode != null)
					this.store(snap_name, snapFolder, secureKey);
				else {
